...
export async function POST(req: Request) {
  ...
  if (taken) {
    if (requested) return NextResponse.json({ error: "That link's already spoken for. Try another." }, { status: 400 });
  ...
  const { data: shop, error } = await supabase
    ...
    .select("id")
    .single();
  if (error || !shop) return NextResponse.json({ error: "That didn't go through. Try again." }, { status: 500 });
