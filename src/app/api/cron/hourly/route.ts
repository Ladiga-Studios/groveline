...
export async function GET(req: Request) {
  ...
  const { data: claims } = await admin
    .from("claims")
    .select("id, buyer_name, buyer_phone, quantity, method, delivery, payment_status, drops!inner(id, title, price_cents, seller_id, profiles!drops_seller_id_fkey(email, notify_on_claim, notify_digest, name))")
  ...
  for (const c of claims ?? []) {
    const d = (Array.isArray(c.drops) ? c.drops[0] : c.drops) as { id: string; title: string; price_cents: number; seller_id: string; profiles: { email: string | null; notify_on_claim: boolean; notify_digest: boolean; name: string } | { email: string | null; notify_on_claim: boolean; notify_digest: boolean; name: string }[] };
    const p = Array.isArray(d.profiles) ? d.profiles[0] : d.profiles;
    if (!p?.notify_on_claim || !p.email) {
      skipIds.push(c.id);
      continue;
    }
    const e = bySeller.get(d.seller_id) ?? { email: p.email, name: p.name, rows: [] };
    e.rows.push(c);
    bySeller.set(d.seller_id, e);
  }
