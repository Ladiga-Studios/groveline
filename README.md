...
Connected accounts are created with: the seller pays Stripe fees, Stripe carries unrecoverable negative balances, Express dashboard, and Stripe collects requirements. This matches the "Create account" form in the Stripe dashboard and the platform's confirmed setup. Express with Stripe-owned losses is a public preview feature on Stripe's side; if account creation ever fails with a version error, set `STRIPE_CONNECT_API_VERSION` to the preview version Stripe names. Stripe bills the platform about $2 per month for each Express account that receives a payout that month.
