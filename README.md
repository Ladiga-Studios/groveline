...
ANTHROPIC_API_KEY=...
RESEND_API_KEY=...
RESEND_FROM="Groveline <hello@send.groveline.io>"
```
...
Card payments are **direct charges** created on the seller's connected account. The charge, the statement descriptor the buyer sees, Stripe's processing fee, and any dispute all belong to the seller, exactly as if they ran the card at their own booth. Groveline takes no application fee.

Connected accounts are Standard: the seller pays Stripe fees, Stripe carries unrecoverable negative balances, the seller gets their own full Stripe dashboard login, and Stripe collects requirements. In the Stripe dashboard, Settings, Connect, Platform setup should show `dashboard: full` (Standard) to match; if it still says express, ask Stripe support to change it. Express with Stripe-owned losses exists as a preview but the API rejects it, so Standard is the supported path. Standard accounts have no per-account monthly fee to the platform.
...
- Spam defenses on reservations: a minimum time-on-page check (a visible "try again", never a silent drop) and optional Cloudflare Turnstile. No honeypot, for the same autofill reason as the support form
